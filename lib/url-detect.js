...
export function detectContext({ url, dom }) {
  ...
  // Ground truth from the DOM probe: if content.js already extracted the
  // case/task info from the form, trust it. URL alone may not be reliable
  // on workspace shells (/now/...) that wrap the record in a frame.
  if (dom && typeof dom === "object") {
    if (dom.type === "case_or_task" && dom.sysId && dom.tableName) {
      const kind = dom.tableName === "sn_customerservice_task" ? "task" : "case";
      return { kind, sysId: dom.sysId, instance, caseNum: dom.caseNum };
    }
    if (dom.type === "kb" && dom.kbNumber) {
      return { kind: "kb", number: dom.kbNumber, sysId: null, instance };
    }
  }
