...
// Runs in the page's MAIN world so it can read window.g_ck (the SN CSRF
// token). SN's /api/now/... endpoints return 401 if X-UserToken is missing
// even when session cookies are valid.
function _pageFetch(url, binary, baseHeaders) {
  return new Promise((resolve) => {
    const headers = Object.assign({}, baseHeaders);
    try {
      // window.g_ck is set by the SN web app on every authenticated page.
      // Without it, REST calls return 401 "User is not authenticated".
      if (typeof window !== "undefined" && window.g_ck) {
        headers["X-UserToken"] = window.g_ck;
      }
    } catch (_) { /* g_ck not available — proceed without */ }

    console.log("[send-to-claude] fetch", url, "g_ck?", !!headers["X-UserToken"]);

    fetch(url, { method: "GET", credentials: "include", headers })
      .then(async (res) => {
        if (!res.ok) {
          const txt = await res.text().catch(() => "");
          console.warn("[send-to-claude] fetch failed", res.status, txt.slice(0, 200));
          resolve({
            ok: false,
