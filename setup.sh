...
VENV_DIR="$REPO_DIR/host/.venv"
VENV_PY="$VENV_DIR/bin/python3"

if [[ -x "$VENV_PY" ]] && "$VENV_PY" -c "import pdfplumber" 2>/dev/null; then
    ok "pdfplumber already installed in venv"
else
    if [[ ! -x "$VENV_PY" ]]; then
        if python3 -m venv "$VENV_DIR" >/tmp/send_to_claude_venv.log 2>&1; then
            ok "Created venv at $VENV_DIR"
        else
            fail "Failed to create venv at $VENV_DIR. See /tmp/send_to_claude_venv.log"
            exit 1
        fi
    fi
    if "$VENV_PY" -m pip install pdfplumber >/tmp/send_to_claude_pip.log 2>&1; then
        ok "pdfplumber installed in venv"
    else
...
# before it can log anything. The launcher invokes the venv Python by
# absolute path, which sidesteps the PATH issue and also ensures the host
# always uses the Python where pdfplumber is installed.
LAUNCHER="$REPO_DIR/host/launcher.sh"
cat > "$LAUNCHER" <<EOF_LAUNCHER
#!/bin/bash
# Auto-generated by setup.sh — do not edit; re-run setup.sh to regenerate.
exec "$VENV_PY" "$HOST_BIN" "\$@"
EOF_LAUNCHER
chmod +x "$LAUNCHER"
...
write_manifest() {
    ...
    mkdir -p "$target_dir"
    # Manifest path points at the launcher wrapper, NOT the .py file
    # directly. See the comment in §6 about Chrome's sanitized
    # environment on macOS.
    python3 - "$HOST_MANIFEST_TEMPLATE" "$LAUNCHER" "$EXT_ID" "$manifest_path" <<'PYEOF'
import json, sys
...
PING_RESULT="$(python3 - "$LAUNCHER" <<'PYEOF'
